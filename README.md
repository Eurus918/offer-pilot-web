...
| 目的 | 怎么做 | 数据在哪 |
|---|---|---|
| **想让同事用起来**（推荐） | 把仓库链接发给他，照上面「跑起来」三步执行 | 在他自己电脑上，你看不到他的数据，他也看不到你的 |
| **只想让他看一眼长什么样** | 发在线演示地址 | 示例数据，注意：**在线演示是共享的**，谁的改动别人都能看到 |

### 你改了代码，云端怎么跟着更新

Render 默认开着 **Auto-Deploy**：往 `main` 推一次提交，它就会自动重新构建部署，通常 2–3 分钟后生效，同事刷新页面即可看到。

```bash
git add -A
git commit -m "你的改动说明"
git push origin main     # 推送后 Render 自动重部署
```

> 需要在 Render 控制台确认服务的 **Auto-Deploy 是 Yes**（Blueprint 创建时默认开启）。
> 免费档重新部署会让演示数据回到 `store.demo.json` 的初始状态。
